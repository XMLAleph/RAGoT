...
@dataclass
class TempAnsweringStep(InferenceStep):
    answers: str
    
@dataclass
class AnswerVotingStep(InferenceStep):
    vote: str
...
class StructuredDataInstance(BasicDataInstance):
    ...
    def add_vote(self, votestep: AnswerVotingStep):
        self.get_current_inference_seq().append(votestep)
    
    def add_temp_answers(self, qtastep: TempAnsweringStep):
        self.get_current_inference_seq().append(qtastep)
    ...
    def get_last_second_step(self):
        try:
            return self.get_current_inference_seq()[-2]
        except:
            print(
                "No more than one step yet"
            )
            return self.get_current_inference_seq()[-1]
    
    def get_last_third_step(self):
        try:
            return self.get_current_inference_seq()[-3]
        except:
            print(
                "No more than one step yet"
            )
    ...
    def get_answers(self):
        ...
        if answers:
            ...
            raise ValueError("No answer generated yet")
    
    '''
    def get_last_qa_answer(self):
        for step in reversed(self.get_current_inference_seq()[1:]):
            if isinstance(step, QuestionAnsweringStep) and step.participant == 'step_by_step_cot_gen':
                return step.answer
    ...
    def get_printable_reasoning_chain(self, indent=""):
        ...
        for step in self["inference_seq"]:
            ...
            if isinstance(step, TempAnsweringStep):
                if chain:
                    chain += "\n"
                chain += indent + "TAs: " + step.answers
            if isinstance(step, AnswerVotingStep):
                if chain:
                    chain += "\n"
                chain += indent + "V: " + step.vote
            if isinstance(step, QuestionAnsweringStep):
                if chain:
                    chain += "\n"
