...
from llm_qa_systems.data_instances import QuestionAnsweringStep
from llm_qa_systems.llm_qa_model import LLMQAModel

from llm_qa_systems.Modules import BasicModule
from llm_qa_systems.Modules import para_to_text, add_and_reorder_if_pinned, get_spacy_object
...
class LLMQAGenerator(BasicModule):
    def __init__(
        self,
        next_model=None,
        end_state="[EOQ]",
    ...
    def update_state(self, answer, state):
        ...
        new_state.data.add_answer(QuestionAnsweringStep(answer=json.dumps(answer,ensure_ascii=False), score=0, participant=state.next))
